<template>
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="领域id">
          <el-input v-model="searchInfo.responsible_dept_id" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item label="业务线名称">
          <el-input v-model="searchInfo.team_name" placeholder="搜索条件" />
        </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="业务线team leader" prop="leader" width="120" />
        <el-table-column align="left" label="领域id" prop="responsible_dept_id" width="120" />
        <el-table-column align="left" label="业务线名称" prop="team_name" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateNocTeamFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getNocTeamList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteNocTeamByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateNocTeamFunc = async(row) => {
    const res = await findNocTeam({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteNocTeamFunc = async (row) => {
    const res = await deleteNocTeam({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
